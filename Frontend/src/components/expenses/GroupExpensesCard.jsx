  ...
  groupExpenses.forEach((expense) => {
    ...
    Object.entries(expense.summary).forEach(([debtor, amount]) => {
      ...
      if (expense.paidBy === sessionUser && debtor !== sessionUser) {
        owedToYouMap[debtor] = (owedToYouMap[debtor] || 0) + amt;
      }

      if (debtor === sessionUser && expense.paidBy !== sessionUser) {
        youOweMap[expense.paidBy] = (youOweMap[expense.paidBy] || 0) + amt;
      }
    });
  });

  owedToYou = Object.entries(owedToYouMap).map(
    ([name, amount]) => `${name} owes you Rs ${amount.toFixed(2)}`
  );

  youOwe = Object.entries(youOweMap).map(
    ([name, amount]) => `You owe ${name} Rs ${amount.toFixed(2)}`
  );
  totalOwedToYou = Object.values(owedToYouMap).reduce((sum, amt) => sum + amt, 0);
  totalYouOwe = Object.values(youOweMap).reduce((sum, amt) => sum + amt, 0);
...
<div className="space-y-4">
  {groupExpenses.map((exp, idx) => {
    ...
    const userOwes = exp.summary[sessionUser]; // your share in this expense
    const isPaidByYou = exp.paidBy === sessionUser;

    let oweText = "";

    if (isPaidByYou) {
      oweText = "You paid — others owe you";
    } else if (userOwes > 0) {
      oweText = `You owe Rs ${userOwes.toFixed(2)}`;
    } else {
      oweText = "You owe none";
    }
      ...
      >
        <div className="card-body space-y-1">
          ...
          <div className="flex justify-between items-start text-sm text-gray-600">
            ...
            <p className="text-right italic">{oweText}</p>
          </div>
